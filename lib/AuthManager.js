...
module.exports = function (server, options) {
    ...
    var statusUrl = options.statusUrl || '/status';
     ...
     * 
     */
    server.get(loginUrl, function (req, res, next) {
        if (hasBasisAuth(req)) {
            var info = req.authorization.basic;
            if (req.query['scope'])
                info.scope = req.query['scope'];
            options.authClient(info, function (err, result) {
                if (err) {
                    return next(err);
                }
                else {
            ...
            res.send(errorsManager.ERROR(errorsCodes.authErrorCodes, {}, "Informations d'authentification incomplètes"));
        }
    });
     ...
     * 
     */
    server.get(logoutUrl, function (req, res, next) {
    ...
    server.get(statusUrl, function (req, res, next) {
